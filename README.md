...
- **阅读设置**：自定义字体、颜色、阅读方向等
- **富文本排版**：支持小说富文本格式渲染
- **阅读进度记忆**：自动记录小说滚动位置，下次继续阅读
- **原生阅读器**：WebView 沉浸式小说阅读体验
- **小说翻译**：集成在线翻译功能

#### 漫画翻译

- **一键翻译**：内置漫画图片翻译引擎，无需外部服务
- **本地推理**：基于 ONNX Runtime Web 的文本检测、OCR 识别与图像修复，全部在浏览器本地完成
- **排版引擎**：支持横排/竖排文字排版，自动匹配气泡与阅读顺序
- **多模型提供商**：支持 SiliconCloud 等多家 LLM 提供商驱动翻译
- **原图/译文切换**：一键对比原图与翻译结果
...
- **本地黑名单**：屏蔽特定标签或用户的作品，支持标签化 UI 管理，可单独移除
...
- **云端同步**：多端数据云同步，PBKDF2 密钥派生 + AES 加密，支持冲突检测与智能合并
