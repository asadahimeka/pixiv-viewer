...
又一个 Pixiv 免登录阅览工具，提供 pixiv 的插画、动图、漫画和小说等作品的在线浏览，适配多端样式，提供多种浏览布局选择，支持 PWA 安装，支持自定义 API 与图床，支持 RefreshToken/OAuth/Cookie 方式登录。
...
- **Vue 2.7**：使用最新的 Vue 2.7 版本，支持 Composition API
...
5. 执行如下命令进行打包，打包完的文件在 `dist` 目录里，可自行部署到服务器
