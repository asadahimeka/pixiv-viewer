<template>
  <div class="illusts">
    <top-bar />
    <h3 class="af_title">{{ $route.params.word }} 的搜索结果</h3>
    ...
    <van-empty v-if="!loading && !userList.length" description="暂无数据" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getUserList: _throttle(async function () {
      const { word } = this.$route.params
      if (!word) return
      this.loading = true
      this.userList = []
      let res = await api.searchUser(word);
      if (res.status === 0) {
