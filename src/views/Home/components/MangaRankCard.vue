<template>
  <div class="rank-card">
    <van-cell class="cell" :border="false" is-link to="/rank/daily_manga">
      <template #title>
      ...
      <swiper class="swipe-wrap" :options="swiperOption">
        ...
        <swiper-slide class="swipe-item more">
          <ImageSlide :images="slides">
            <div class="link" @click="$router.push('/rank/daily_manga')">
...
export default {
  ...
  methods: {
    async getRankList() {
      this.loading = true
      const res = await api.getRankList('day_manga')
      if (res.status === 0) {
