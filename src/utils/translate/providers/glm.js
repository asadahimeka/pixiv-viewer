...
export async function translate(regions, config) {
  ...
  if (!apiKey) {
    throw new ProviderError(
      'GLM/Kimi API key is required. Provide apiKey in config.',
      'INVALID_API_KEY'
    )
  }
  ...
  try {
    ...
    throw new ProviderError(
      `Network error calling GLM API: ${err.message}`,
      'NETWORK_ERROR',
  ...
  if (!response.ok) {
    ...
    throw new ProviderError(
      `GLM API returned status ${response.status}: ${errorBody || response.statusText}`,
  ...
  try {
    ...
    throw new ProviderError(
      `Failed to parse GLM API response: ${err.message}`,
      'MALFORMED_RESPONSE',
      { originalError: err.message }
    )
  }
  ...
  if (!content) {
    throw new ProviderError(
      'GLM API returned empty response content',
      'MALFORMED_RESPONSE',
      { rawResponse: json }
    )
  }
